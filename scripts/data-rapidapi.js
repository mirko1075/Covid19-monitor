...
class PrintData {
  ...
  printCountriesDropDown = (data) => {
    ...
    for (let i = 0; i < listCountriesArr.length; i++) {
      dropDownHTML += `<option value="${listCountriesArr[i].country}" >${listCountriesArr[i].country}</option>`;
    }
    dropDownHTML += `</select>`;
            ...
            countryDetailText = `<table>
                <thead>
                      ...
                      <th colspan="3" class="result">
                        <table>
                          <tr>
                            <td><img src="${countriesArr[i].flag}" class="flag"></td>
